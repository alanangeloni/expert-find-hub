...
const InvestmentFirmDetailPage = () => {
  ...
  if (error || !firm) {
    return <div className="min-h-screen bg-slate-50 p-6 flex flex-col items-center justify-center text-center">
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container max-w-6xl mx-auto py-8 px-4 md:px-6">
          ...
          <div className="container mx-auto p-6 sm:p-8">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-8">
              <div className="h-16 w-16 md:h-24 md:w-24 bg-slate-100 rounded-lg flex items-center justify-center">
                {firm.logo_url ? <img src={firm.logo_url} alt={firm.name} className="h-12 w-12 md:h-16 md:w-16 object-contain" /> : <Building className="h-8 w-8 md:h-10 md:w-10 text-slate-400" />}
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl md:text-4xl font-bold">{firm.name}</h1>
                  {firm.verified && <span className="text-blue-500 mt-1 md:mt-2">
                ...
                <div className="flex items-center gap-2 mt-2">
                  {firm.rating && <div className="flex">
                      {[...Array(5)].map((_, i) => <Star key={i} className={`h-4 w-4 md:h-5 md:w-5 ${i < Math.floor(Number(firm.rating) || 0) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`} />)}
                    </div>}
                  {firm.rating && <span className="text-sm md:text-base font-medium">{firm.rating.toFixed(1)}</span>}
                  {firm.rating && firm.review_count && <span className="text-xs md:text-sm text-gray-500">({firm.review_count} reviews)</span>}
                </div>
                
                {firm.description && <div className="mt-2 text-sm md:text-base text-gray-600">{firm.description}</div>}
                
                <div className="flex flex-wrap gap-2 mt-3">
                  {firm.asset_classes?.map((assetClass: string, index: number) => <Badge key={index} variant="secondary" className="bg-slate-100">
                      {assetClass}
                    </Badge>)}
                </div>
              </div>
              ...
              <div className="w-full md:w-auto flex flex-col gap-3 mt-4 md:mt-0">
                ...
                <Button asChild size="lg" className="bg-mint-500 hover:bg-mint-600 text-white">
                  <a href={firm.firm_link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center">
                    Invest Now
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6 container mx-auto px-4">
          {firm.website_url && <Button asChild size="lg" variant="outline" className="text-brand-blue border-brand-blue/50 hover:bg-brand-blue/5 hover:text-brand-blue hover:border-brand-blue flex-grow sm:flex-grow-0 min-w-[150px]">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-2/3 space-y-8">
            <Tabs defaultValue="overview" className="w-full">
              ...
              <TabsContent value="overview" className="mt-2 space-y-6">
                {(firm.long_description || firm.about) && <Card>
                    <CardHeader>
                ...
                {(moneyMakingMethods.length > 0 || firm.how_company_makes_money) && <Card>
                        <CardHeader>
                            <CardTitle className="text-xl flex items-center">
                        ...
                        <CardContent className="space-y-4">
                            {moneyMakingMethods.map((method, index) => <div key={method.id || index}>
                            ...
                            {firm.how_company_makes_money && <div className="prose prose-slate max-w-none text-slate-700 leading-relaxed">
                                    <p>{firm.how_company_makes_money}</p>
                                </div>}
                        </CardContent>
                    </Card>}
                {firm.investment_risks && <Card>
                        <CardHeader>
                ...
                {clientTypes.length > 0 && <Card>
                    ...
                    <CardContent>
                        ...
                        {clientTypes.map((type, index) => <Badge key={index} variant="secondary" className="bg-slate-100 text-slate-700 font-normal px-2.5 py-1">
                            {type.name}
                            </Badge>)}
                        </div>
                    </CardContent>
                    </Card>}
                {registrations.length > 0 && <Card>
                    <CardHeader>
                    ...
                    <CardContent>
                        <div className="flex flex-wrap gap-2">
                        {registrations.map((reg, index) => <Badge key={index} variant="outline" className="border-slate-300 text-slate-700 font-normal px-2.5 py-1">
                            {reg.name}
                            </Badge>)}
                        </div>
                    </CardContent>
                ...
                {leadership.length > 0 ? <Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                        {leadership.map((person, index) => <div key={index} className="flex items-start gap-4 p-4 border rounded-lg bg-slate-50/50">
                            ...
                            <div className="flex-1">
                              ...
                              {person.linkedin_url && <a href={person.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:underline mt-1 inline-flex items-center">
                  ...
                  </Card> : <div className="text-center py-8 text-slate-600">
          ...
          <div className="lg:w-1/3 space-y-6">
            ...
            {similarFirms.length > 0 && <Card>
                <CardHeader>
                    <CardTitle className="text-lg">Similar Investment Firms</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  {similarFirms.slice(0, 3).map((similarFirm: any) => <Link key={similarFirm.id} to={`/firms/${similarFirm.slug}`} className="block group p-3 rounded-lg border hover:bg-mint-100 transition-colors">
